...
for ambig in ambigs:
    ...
    if ambig != "":
        ambig = ambig[:-1]+"_"
    with open("data/"+ambig+"trajectories","rb") as f:
    ...
    #extract policy
    policy = [learner.predict(i,deterministic=True)[0].item() for i in range(100)]
    with open("data/"+ambig+"policy",'wb') as f:
